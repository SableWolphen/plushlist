...
const checks = [
  ...
  [today.includes("homeSettings.insights") && today.includes("homeSettings.extras"), "people can keep deep smart insights and extra Today tools off the main surface without deleting data"],
  ...
  [today.includes('dayType === "soft"') && today.includes("smartNextStepHidden ? null") && today.includes("setSmartNextStepHidden(Boolean(hidden))"), "Next Step stays Soft-Day-only and Hide for today suppresses it immediately"],
  [today.includes("DayModeCue") && today.includes("Whole routine") && today.includes("Gentler versions + one next step") && today.includes("Smallest meaningful steps"), "Full, Soft and Tiny days clearly explain how the remaining day changes"],
