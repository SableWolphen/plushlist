...
const checks = [
  ...
  [baby.includes("✓ {completedCount} tucked in today") && baby.includes("remainingJobs.slice(0, 3)"), "Baby Mode keeps completed work collapsed and limits the first job list"],
  [baby.includes("row.key !== nextStepTask.key") && baby.includes("useState(true)"), "Baby Mode shows tasks immediately without repeating the primary tiny thing"],
  [today.includes("<PlushKnowsMe {...modeProps} quiet") && today.includes("<LazySmartAdaptationPanel {...modeProps} quiet"), "Baby Mode keeps learning engines active without the normal Today recommendation stack"],
