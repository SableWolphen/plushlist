...
const mustContain = {
  ...
  'src/components/baby-today.jsx': [
    ['onClick={openCare}', 'Baby Mode help/Care route'],
    ['onClick={() => openTaskManager?.()}', 'Baby Mode edit jobs action'],
    ['role="tablist" aria-label="Baby Mode view"', 'Baby Mode Schedule/Little Jobs tabs'],
    ['onClick={() => setTodayCardIndex?.(0)}', 'Baby Mode Schedule tab'],
    ['onClick={() => setTodayCardIndex?.(1)}', 'Baby Mode Little Jobs tab'],
    ...
    ['babyScheduleEntries.map((entry, index)', 'Baby Mode full schedule list'],
    ['waiting.map((task, index)', 'Baby Mode Little Jobs list'],
    ['onClick={() => !isFutureView && toggle(task.key)}', 'Baby Mode task completion toggle'],
    ['onClick={() => selectTaskPreviewDate?.(date)}', 'Baby Mode week date selector'],
    ['I need a little help', 'Baby Mode help entry point'],
  ],
