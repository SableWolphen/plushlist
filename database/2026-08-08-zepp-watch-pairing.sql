-- Secure, revocable pairing for the PlushLife Zepp OS companion.
-- Pairing secrets are never stored in plaintext and the table is not exposed
-- directly to clients. All watch traffic is mediated by the watch-sync Edge
-- Function.
...
create index if not exists watch_pairings_user_idx
  on public.watch_pairings(user_id)
...
as $$
declare
  pairing public.watch_pairings;
begin
  if auth.uid() is null then
    raise exception 'Sign in before connecting a watch';
  end if;
...
comment on table public.watch_pairings is
  'Hashed Zepp watch credentials and temporary pairing codes; accessible only through controlled functions.';
comment on function public.claim_watch_pairing(text) is
  'Claims an unexpired Zepp pairing code for the currently authenticated PlushLife user.';
