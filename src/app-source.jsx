...
function GlowUpTracker() {
  ...
  const [supportMoodSummary, setSupportMoodSummary] = useState(null);
  ...
  const loadSupportOwner = async (targetOwnerId) => {
    ...
    const [notesResult, rewardsResult, progressResult, historyResult, tasksResult, schedulesResult, exceptionsResult, moodResult, suggestionsResult, achievementsResult, restResult] = await Promise.all([
      ...
      supabase.rpc("guardian_daily_checkin_summary", { p_owner_user_id: targetOwnerId, p_date: period.date }).maybeSingle(),
    ...
    if (notesResult.error || rewardsResult.error || progressResult.error || historyResult.error || tasksResult.error || schedulesResult.error || exceptionsResult.error || moodResult.error || suggestionsResult.error) {
  ...
  // PlushHome must always show TODAY's schedule. Do not let a previously
  // selected Tasks/Calendar day leak into the Home preview.
  const homeScheduleDayId = dayIdForDate(period.date);
  const homeSelectedSchedule = personalSchedules.find((item) => item.day_id === homeScheduleDayId) || null;
  const homeScheduleExceptionEntries = scheduleExceptions
    .filter((item) => item.start_date <= period.date && item.end_date >= period.date)
    .flatMap((item) => (item.entries || []).map((entry) => ({ ...entry, isException: true })));
 ...
 */
      ...
      {autoPopupToShow === "check_in" && (
        <div role="dialog" aria-modal="true" aria-labelledby="checkin-popup-title" onClick={() => { setCheckInPopupOpen(false); setCheckInPopupDismissedToday(true); }} style={{ position: "fixed", inset: 0, zIndex: 60, display: "grid", placeItems: "center", padding: 18, background: "rgba(64,39,80,.46)", backdropFilter: "blur(5px)" }}>
          <div onClick={(event) => event.stopPropagation()} style={{ width: "min(100%, 520px)", maxHeight: "min(88vh,760px)", overflowY: "auto", padding: "22px 20px", borderRadius: 26, background: "linear-gradient(160deg,#FFFDFE,#FFF0FA 58%,#EBFBFF)", border: "2px solid #D994E7", boxShadow: "0 24px 80px rgba(61,35,78,.3)" }}>
            ...
            <div style={{ marginTop: 4, fontSize: 11.5, lineHeight: 1.45, color: "#6B7C99" }}>Pick the day size that fits. Progress stays safe.</div>
  ...
  <button type="button" onClick={() => setPendingTaskDelete(null)} style={{ padding: "8px 12px", borderRadius: 10, border: "1px solid #D8C8E2", background: "white", color: "#76558A", fontWeight: 900 }}>Keep task</button>
      ...
      <div
        ...
        {false && user && (betaBannerDismissed ? (
        ...
        <style>{`
          .pl-unified-page-shell{display:grid;gap:8px;margin:0 auto;max-width:760px;color:#62506D;padding-bottom:calc(104px + env(safe-area-inset-bottom))}
          .pl-unified-page-hero{position:relative;overflow:hidden;margin:-2px 0 1px;padding:12px 13px 13px;min-height:126px;border-radius:20px;background:linear-gradient(145deg,#FFF7FA 0%,#FAEDFB 55%,#F2ECFF 100%);border:1px solid #EBD9F0;box-shadow:0 10px 28px rgba(101,63,115,.055)}
          ...
          .pl-unified-page-hero:after{content:"🧸";position:absolute;right:12px;bottom:6px;font-size:42px;opacity:.16;transform:rotate(8deg);filter:drop-shadow(0 5px 8px rgba(83,45,98,.1))}
          ...
          .pl-unified-wordmark{font-family:Georgia,"Times New Roman",serif;font-style:italic;font-size:21px;font-weight:900;color:#4A235F;letter-spacing:-1px}.pl-unified-wordmark span{color:#DB72B4}
          .pl-unified-actions{display:flex;align-items:center;gap:7px}.pl-unified-date{padding:6px 8px;border-radius:999px;background:rgba(255,255,255,.84);border:1px solid #E9D6EE;color:#6B4E75;font-size:10px;font-weight:900}
          ...
          .pl-unified-page-title{margin-top:9px;max-width:78%}.pl-unified-kicker{font-size:9.6px;letter-spacing:.14em;font-weight:950;color:#B653C5}.pl-unified-page-title h2{margin:2px 0 0;font-size:19px;line-height:1.05;color:#472C54;letter-spacing:-.5px}.pl-unified-page-title p{margin:4px 0 0;color:#7C6684;font-size:10px;line-height:1.4}
          ...
          @media(max-width:520px){.pl-unified-page-shell{padding-bottom:calc(86px + env(safe-area-inset-bottom));gap:7px}.pl-app-bottom-nav{min-height:48px;padding-top:2px}.pl-app-nav-btn{min-height:34px;font-size:7.5px}.pl-app-nav-btn .ico{font-size:13px}.pl-app-nav-add{width:36px;height:36px;min-height:36px;font-size:19px;margin-top:-9px}.pl-unified-page-hero{min-height:108px;padding:9px 10px 10px;border-radius:17px}.pl-unified-wordmark{font-size:18px}.pl-unified-date{font-size:8px;padding:5px 7px}.pl-unified-gear{width:32px;height:32px;font-size:14px}.pl-unified-page-title{margin-top:7px;max-width:80%}.pl-unified-kicker{font-size:8.4px}.pl-unified-page-title h2{font-size:17px}.pl-unified-page-title p{font-size:9.2px;line-height:1.3}.pl-unified-page-hero:after{font-size:36px;right:8px;bottom:5px}}
        `}</style>
