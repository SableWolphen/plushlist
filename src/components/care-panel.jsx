...
export function CarePanel(props) {
  ...
  void pathFeedbackVersion;

  const markCare = (text, kind = "care") => { addCaringDay(userId, localDay(), kind); recordMoment(userId, text, kind); };
  const startCare = (toolId) => { const tool = COMFORT_TOOLS.find((entry) => entry.id === toolId); markCare(tool ? `You chose ${tool.name} when you needed support.` : "You chose a care tool instead of pushing through alone.", "calm"); props.openCareSession(toolId); };
  const startSleep = (toolId) => { const tool = SLEEP_TOOLS.find((entry) => entry.id === toolId); markCare(tool ? `You made room to wind down with ${tool.title}.` : "You made room for sleep support.", "sleep"); props.setSleepToolOpen(toolId); };
  ...
  return <div data-plushcare-redesign="true" style={{ marginBottom: 18, display: "grid", gap: 9 }}>
    ...
    {goldMemoryUnlocked && <section style={{ ...shellCard, padding: "10px 11px", background: "linear-gradient(145deg,#F8F2FF,#FFFFFF)", borderColor: "#E3D2EC" }}><div style={{ fontSize: 9.7, letterSpacing: ".11em", fontWeight: 900, color: "#8E4EAA" }}>✨ GOLD · PLUSHMEMORY</div>{memory.tool ? <div style={{ marginTop: 5, display: "flex", gap: 8, justifyContent: "space-between", alignItems: "center" }}><div style={{ minWidth: 0, fontSize: 10.5, lineHeight: 1.42, color: "#806B8D" }}><strong>{memory.tool.name}</strong> has helped {memory.count} {memory.count === 1 ? "time" : "times"} when you checked afterward. {memory.confidence === "strong" ? "That is becoming a useful personal pattern." : "PlushLife is treating this as a growing clue, not a rule."}</div><button type="button" onClick={() => startCare(memory.tool.id)} style={smallButton}>Use again</button></div> : <div style={{ marginTop: 4, fontSize: 10.4, lineHeight: 1.4, color: "#806B8D" }}>Still learning what works for you. After a care session, tell PlushLife whether it helped and the pattern will get more personal.</div>}</section>}
