...
import { addCaringDay, localDay, recordMoment, sleepMemory, supportMemory } from "../plush-memory.js";
...
export function CarePanel(props) {
  ...
  return <div data-plushcare-redesign="true" style={{ marginBottom: 18, display: "grid", gap: 9 }}>
    ...
    {goldMemoryUnlocked && <section style={{ ...shellCard, padding: "10px 11px", background: "linear-gradient(145deg,#F8F2FF,#FFFFFF)", borderColor: "#E3D2EC" }}>
      <div style={{ fontSize: 9.7, letterSpacing: ".11em", fontWeight: 900, color: "#8E4EAA" }}>✨ GOLD · PLUSHMEMORY</div>
      {memory.tool ? <div style={{ marginTop: 5, display: "flex", gap: 8, justifyContent: "space-between", alignItems: "center" }}><div style={{ minWidth: 0, fontSize: 10.5, lineHeight: 1.42, color: "#806B8D" }}><strong>{memory.tool.name}</strong> has helped {memory.count} {memory.count === 1 ? "time" : "times"} when you checked afterward. {memory.confidence === "strong" ? "That is becoming a useful personal pattern." : "PlushLife is treating this as a growing clue, not a rule."}</div><button type="button" onClick={() => startCare(memory.tool.id)} style={{ minHeight: 44, padding: "7px 9px", borderRadius: 10, border: "1px solid #D7BFE4", background: "white", color: "#76558A", fontWeight: 900, fontSize: 10.2, cursor: "pointer", flexShrink: 0 }}>Use again</button></div> : <div style={{ marginTop: 4, fontSize: 10.4, lineHeight: 1.4, color: "#806B8D" }}>Still learning what works for you. After a care session, tell PlushLife whether it helped and the pattern will get more personal.</div>}
    </section>}
    ...
    <section aria-label="PlushCare main spaces" style={{ borderRadius: 14, border: "1px solid #E6D4F2", background: "rgba(255,255,255,.72)", padding: "9px" }}>
      ...
      {goldMemoryUnlocked && <div style={{ margin: "0 3px 8px", padding: "8px 9px", borderRadius: 10, background: "#242D58", color: "#DCE3FA", border: "1px solid #3B4A85" }}><div style={{ fontSize: 9.4, letterSpacing: ".1em", fontWeight: 900, color: "#A9B9F5" }}>🌙 TONIGHT</div><div style={{ marginTop: 3, fontSize: 10.2, lineHeight: 1.4 }}>{sleep.text}</div>{sleep.tool && <button type="button" onClick={() => { props.setCareSection("sleep"); startSleep(sleep.tool.id); }} style={{ marginTop: 6, minHeight: 44, padding: "6px 9px", borderRadius: 9, border: "1px solid #6D80C3", background: "#344173", color: "white", fontWeight: 900, fontSize: 10.1, cursor: "pointer" }}>Try {sleep.tool.title}</button>}</div>}
