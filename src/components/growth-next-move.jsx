...
function chooseMove(state) {
  ...
  if (dayModel?.intervention && dayModel.intervention.kind !== "steady") {
    ...
    return {
      ...
      reason: "Your recent day pattern changed enough for PlushLife to recommend a different amount of effort.",
    };
  }
...
function learningSummary(state) {
  ...
  const learning = profiles.filter((profile) => profile.confidence === "learning");
  const lines = [];
  ...
  return { knows: lines.slice(0, 3), unsure };
}
    ...
    <>
      <section aria-label="Best next adjustment" style={{ marginBottom: 12, padding: "13px 14px", borderRadius: 16, border: "1px solid #D8E7E2", background: "linear-gradient(145deg,#F6FCFA,#FFF9FD)" }}>
        ...
        <details style={{ marginTop: 7 }}>
          <summary style={{ minHeight: 44, display: "flex", alignItems: "center", cursor: "pointer", color: "#765F84", fontSize: 11, fontWeight: 900 }}>Why this suggestion?</summary>
          <div style={{ padding: "7px 9px", borderRadius: 10, background: "rgba(255,255,255,.68)", color: "#766981", fontSize: 10.7, lineHeight: 1.45 }}><strong>Why PlushLife changed this:</strong> {move.reason}<div style={{ marginTop: 4, color: "#8A9A95" }}>{move.evidence}</div></div>
        </details>
      </section>
      ...
      {history.length > 1 && <details style={{ marginBottom: 12, padding: "9px 12px", borderRadius: 14, border: "1px solid #E6DCEF", background: "#FBF8FC" }}>
        <summary style={{ minHeight: 44, display: "flex", alignItems: "center", cursor: "pointer", color: "#765F84", fontSize: 11.2, fontWeight: 900 }}>🧭 What PlushLife changed and why</summary>
        <div style={{ display: "grid", gap: 7, marginTop: 5 }}>{history.slice(-5).reverse().map((item, index) => <div key={`${item.at}-${index}`} style={{ padding: "8px 9px", borderRadius: 10, background: "white", border: "1px solid #EEE6F1" }}><div style={{ fontSize: 11, fontWeight: 900, color: "#66536F" }}>{item.title}</div><div style={{ marginTop: 2, fontSize: 10.3, lineHeight: 1.4, color: "#8A7895" }}>{item.reason || item.text}</div></div>)}</div>
      </details>}
    </>
  );
}
