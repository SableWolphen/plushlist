import { addCaringDay, clearFutureNote, dayForecast, getFutureNote, localDay, recordMoment, registerVisit, rescueSignal, saveFutureNote } from "../plush-memory.js";
...
export function PlushKnowsMe({ user, rows = [], viewDone = {}, dailyCheckIn = {}, dailyCheckInHistory = [], goToDashboard }) {
  ...
  return <section data-contextual-today-support="true" aria-label="Helpful suggestions for today" style={{ display: "grid", gap: 8, margin: "8px 0 10px" }}>
    ...
    {signal.shouldOffer && !rescueDismissed && <div style={{ ...card, borderColor: "#D9C9EB", background: "linear-gradient(145deg,#F8F2FF,#FFFFFF)" }}><div style={{ marginTop: 1, fontSize: 12, fontWeight: 900, color: "#5B4B6B" }}>🫧 Make today smaller?</div><div style={{ marginTop: 3, fontSize: 10.5, lineHeight: 1.45, color: "#806B8D" }}>{signal.reason}</div><div style={{ marginTop: 4, fontSize: 9.7, color: "#9A86A5" }}>Nothing is deleted. This only changes what the day asks you to look at right now.</div><div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginTop: 7 }}><button type="button" onClick={() => openRescue("rescue")} style={{ ...button, border: 0, background: "#8E69B1", color: "white" }}>Yes, make it smaller</button><button type="button" onClick={() => setRescueDismissed(true)} style={button}>Not now</button></div>{feedbackTarget === "rescue" && <FeedbackRow label="Did making the day smaller help?" onFeedback={(feedback) => recordFit("rescue", "make-day-smaller", feedback)} />}</div>}
