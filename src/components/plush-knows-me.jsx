...
export function PlushKnowsMe({ user, rows = [], viewDone = {}, dailyCheckIn = {}, dailyCheckInHistory = [], goToDashboard }) {
  ...
  return <section data-plush-knows-me="true" aria-label="PlushMemory" style={{ display: "grid", gap: 8, margin: "8px 0 10px" }}>
    {showReturn && <div style={{ ...card, borderColor: "#CFE8E1", background: "linear-gradient(145deg,#F2FFFB,#FFF9FD)" }}><div style={{ fontSize: 12.2, fontWeight: 900, color: "#4D8174" }}>🧸 Hi. Your stuff is still here.</div><div style={{ marginTop: 3, fontSize: 10.7, lineHeight: 1.45, color: "#71857F" }}>It has been a few days. There is nothing to catch up on. You can make today tiny and start from right now.</div><div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginTop: 7 }}><button type="button" onClick={openRescue} style={{ ...button, border: 0, background: "#4D9A86", color: "white" }}>🌿 Make today smaller</button><button type="button" onClick={() => goToDashboard?.("care")} style={button}>💜 Open PlushCare</button></div></div>}

    <div data-plush-day-forecast="true" style={{ ...card, borderColor: forecast.mode === "tiny" ? "#CFE8E1" : "#D7DCEF", background: forecast.mode === "tiny" ? "linear-gradient(145deg,#F2FFFB,#FFFFFF)" : "linear-gradient(145deg,#F8F9FF,#FFFFFF)" }}><div style={{ fontSize: 9.5, letterSpacing: ".11em", fontWeight: 900, color: "#6675A2" }}>🌦️ DAY FORECAST</div><div style={{ marginTop: 4, fontSize: 11.7, fontWeight: 900, color: "#5B5973" }}>{forecast.icon} {forecast.title}</div><div style={{ marginTop: 3, fontSize: 10.3, lineHeight: 1.43, color: "#77758A" }}>{forecast.text}</div>{["tiny","soft"].includes(forecast.mode) && <button type="button" onClick={openRescue} style={{ ...button, marginTop: 7 }}>Make today match this</button>}</div>

    {signal.shouldOffer && !rescueDismissed && <div style={{ ...card, borderColor: "#D9C9EB", background: "linear-gradient(145deg,#F8F2FF,#FFFFFF)" }}><div style={{ fontSize: 9.7, letterSpacing: ".11em", fontWeight: 900, color: "#8E4EAA" }}>🫧 PLUSHMEMORY NOTICED</div><div style={{ marginTop: 4, fontSize: 12, fontWeight: 900, color: "#5B4B6B" }}>Want me to make today smaller?</div><div style={{ marginTop: 3, fontSize: 10.5, lineHeight: 1.45, color: "#806B8D" }}>{signal.reason}</div><div style={{ marginTop: 4, fontSize: 9.7, color: "#9A86A5" }}>Nothing is deleted. This only changes what the day asks you to look at right now.</div><div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginTop: 7 }}><button type="button" onClick={openRescue} style={{ ...button, border: 0, background: "#8E69B1", color: "white" }}>Yes, make it smaller</button><button type="button" onClick={() => setRescueDismissed(true)} style={button}>Not now</button></div></div>}

    <div style={{ ...card, padding: "9px 11px" }}><div style={{ display: "flex", justifyContent: "space-between", gap: 8, alignItems: "center" }}><div><div style={{ fontSize: 10.7, fontWeight: 900, color: "#765F84" }}>💞 Care streak · {streak} {streak === 1 ? "day" : "days"}</div><div style={{ marginTop: 2, fontSize: 9.8, lineHeight: 1.4, color: "#8C7A96" }}>Check-ins, one caring step, rest, and asking for help can all count. This is not a productivity streak.</div></div></div></div>
