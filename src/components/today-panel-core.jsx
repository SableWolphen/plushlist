                  ...
                  <div key={r.key}>
                    ...
                    <div className="plushlife-task-row"
                      ...
                      <div onClick={() => { if (isFutureView) return; expandable ? setOpenRow(expanded ? null : r.key) : toggle(r.key); }}
                        ...
                        {arrangeTodayTasks && draggableTodayTask && (() => {
                          const peers = visibleRows.filter((item) => item.sourceTask && item.sourceTask.section === r.sourceTask.section && !viewDone[item.key] && !isFutureView && !isHistoricalView);
                          const peerIndex = peers.findIndex((item) => item.key === r.key);
                          return <span style={{ display: "flex", gap: 3, flex: "0 0 auto" }}>
                            <button type="button" disabled={peerIndex <= 0} onClick={(event) => { event.stopPropagation(); if (peerIndex > 0) moveTrackerTask(r.sourceTask.task_key, -1); }} aria-label={`Move ${r.label} up`} title="Move up" style={{ width: 30, height: 34, minHeight: 34, padding: 0, borderRadius: 8, border: "1px solid #E7D2E8", background: "rgba(255,255,255,.55)", color: day.accent, opacity: peerIndex <= 0 ? .3 : 1, fontWeight: 900, cursor: peerIndex <= 0 ? "default" : "pointer" }}>↑</button>
                            <button type="button" disabled={peerIndex < 0 || peerIndex >= peers.length - 1} onClick={(event) => { event.stopPropagation(); if (peerIndex >= 0 && peerIndex < peers.length - 1) moveTrackerTask(r.sourceTask.task_key, 1); }} aria-label={`Move ${r.label} down`} title="Move down" style={{ width: 30, height: 34, minHeight: 34, padding: 0, borderRadius: 8, border: "1px solid #E7D2E8", background: "rgba(255,255,255,.55)", color: day.accent, opacity: peerIndex < 0 || peerIndex >= peers.length - 1 ? .3 : 1, fontWeight: 900, cursor: peerIndex < 0 || peerIndex >= peers.length - 1 ? "default" : "pointer" }}>↓</button>
                          </span>;
                        })()}
